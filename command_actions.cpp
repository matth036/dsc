...
void messier_catalog_point_to( char* yytext, int yyleng ){
  ...
  if (n != 1) {
    return; /* man sscanf indicates the return value is the number of succesfully assigned items. */
  }
 ...
 * If so, after centering the object, call this function for prompt to keep it centered.
*/
...
void proximate_navigation_stars_view(){
  auto lcd = check_out_main_lcd();
  Simple_Altazimuth_Scope* scope = get_main_simple_telescope();
  std::unique_ptr<Proximate_Navigation_Stars_View> view = 
    std::unique_ptr<Proximate_Navigation_Stars_View>( new Proximate_Navigation_Stars_View( scope ) );
